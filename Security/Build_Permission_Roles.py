...
permission_schema_df['Build Group Tree']            = permission_schema_df.apply(lambda x: f'Grant role {x["Role_Name"]} to role {x["Role_Groups"]};'if x['PARENT_HIERARCHY']!= "ADMIN" else (f'Grant role {x["Role_Groups"]} to role PROD_ADMIN;'),axis = 1 )
